...
void DynamicEnvironments(int expt, int penaltytype)
{
  ...
  sprintf(str, "rev-env-switch-%i-%i-%.4f-%i-%i-%i.txt", NPOP, NOFF, ALPHA, NPARAM, expt, penaltytype);
  fp1 = fopen(str, "w");
      ...
      for(epsilon = 0; epsilon < 1.5; epsilon += epsilonstep)
	{
	  ...
	  for(it = 0; it <= NIT; it++)
	      ...
	      for(i = 0; i < NPOP; i++)
		{
		  ...
		  switch(expt)
		    {
		    case 2: C[i].mu = 1e-8; C[i].mumu = 0; break;
	            case 3: C[i].mu = 1e-7; C[i].mumu = 0; break;
		    case 4: C[i].mu = 1e-6; C[i].mumu = 0; break;
	      ...
	      if(it == 0 && (envswitch == 0 || envswitch == 20 || envswitch == 40) && (fabs(epsilon-0.1) < 0.001 || fabs(epsilon-0.2) < 0.001 || fabs(epsilon-0.5) < 0.001))
		{
		  ...
		  sprintf(str, "rev-env-switch-%i-%i-%.4f-%i-%i-%i-switch-%i-eps-%.4f.txt", NPOP, NOFF, ALPHA, NPARAM, expt, penaltytype, envswitch, epsilon);
		  fp = fopen(str, "w");
		  for(t = 0; t < MAXT; t++)
		    {
		      fprintf(fp, "%i %.4e %.4e %.4e %.4e %.4e %.4e\n", t, outC[t].h, outC[t].hd, outC[t].mu, outC[t].mumu, outC[t].sigma, outC[t].sigmamu);
		    }
		  fclose(fp);
		  ...
		  if(expt == 1)
		    {
		      ...
		      sprintf(str, "rev-env-switch-%i-%i-%.4f-%i-%i-%i-single-hist-switch-%i-eps-%.4f.txt", NPOP, NOFF, ALPHA, NPARAM, expt, penaltytype, envswitch, epsilon);
		      fp = fopen(str, "w");
		      for(t = 0; t < MAXT; t++)
			{
			  for(i = 0; i < NHIST; i++)
...
void Adaptation(int expt, int penaltytype)
{
  ...
  // open summary file for output
  sprintf(str, "rev-adaptation-%i-%i-%.4f-%i-%i-%i.txt", NPOP, NOFF, ALPHA, NPARAM, expt, penaltytype);
  fp = fopen(str, "w");
	  ...
	  for(mu = 0; mu <= 1e-2; mu *= 2) 
	      ...
	      for(it = 0; it < NIT; it++)
		{
		  ...
		  if(expt == 1 && penaltytype == 0)
		    {
		      sprintf(str, "rev-adaptation-%i-%i-%.4f-%i-%i-%i-hist-%.4f-%.4f-%.4e.txt", NPOP, NOFF, ALPHA, NPARAM, expt, penaltytype, sigma, epsilon, mu);
		      fp1 = fopen(str, "w");
		      for(t = 0; t < MAXT; t++)
			{
			  for(i = 0; i < NHIST; i++)
...
void AdaptationTime(int expt, int penaltytype)
{
  ...
  // open summary file for output
  sprintf(str, "rev-adaptationtime-%i-%i-%.4f-%i-%i-%i.txt", NPOP, NOFF, ALPHA, NPARAM, expt, penaltytype);
  fp = fopen(str, "w");
